...
class ColorProductController extends AbstractRestfulController
{
    private $colorProductTable;
    private $form;
    private $responseService;
    private $filterService;
    private $logger;

    public function __construct(ResponseService $responseService, ColorProductTable $colorProductTable)
    {
        $this->responseService = $responseService;
        $this->colorProductTable = $colorProductTable;
    }
    ...
    public function getAction()
    {
        ...
        if($request->isGet()){
            ...
            try {
                ...
                $colorProduct = $this->colorProductTable->fetch($arrParams);
                if(!empty($colorProduct)){
                    $this->responseService->setData($colorProduct);
            ...
            try {
                ...
                if ($this->form->isValid()) {
                    ...
                    if (empty($colorProduct)) {
                        ...
                        if (is_array($colorProduct)) {
                            $this->responseService->setCode(ResponseService::CODE_ALREADY_EXISTS);
                        } else {
                            $this->responseService->setCode(ResponseService::CODE_ERROR);
                            $this->logger->setMethodAndLine(__METHOD__, __LINE__);
                            $this->logger->save(Logger::LOG_APPLICATION,Logger::WARNING,$colorProduct);
                        }
                    }
                } else {
        ...
        if ($request->isPost()) {
            ...
            try {
                ...
                if ($this->form->isValid()) {
                    $arrParams = $this->filterService->setData($arrParams)->getData();
                    $colorProduct = $this->colorProductTable->fetchRow($arrParams);
                    if (is_array($colorProduct) && !empty($colorProduct)) {
                        $arrSet = $this->filterService->getArraySet();
                        $arrWhere = $this->filterService->getArrayWhere();
                        $returnUpdate = $this->colorProductTable->update($arrSet,$arrWhere);
